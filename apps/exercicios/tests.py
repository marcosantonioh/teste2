...
class ResolverExercicioResumoEstacaoTests(TestCase):
    def setUp(self):
        ...
        self.modulo = Modulo.objects.create(nome='Módulo Teste', descricao='Desc', ordem=1)
        self.secao = Secao.objects.create(nome='Seção Teste', modulo=self.modulo, ordem=1)
        self.estacao = Estacao.objects.create(nome='Estação Teste', secao=self.secao, status='livre')
